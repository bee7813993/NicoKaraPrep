...
public class EmojiOptionsTests
{
    ...
    public void オプション解析()
    {
        var e = new TimeTagTool.Core.Model.EmojiEntry
        {
            ReplaceChar = "（花帆）",
            ImageBefore = "a.png",
    ...
    public void アイコン実寸から幅を計算()
    {
        ...
        try
        {
            Assert.True(TimeTagTool.Core.Formats.ImageSizeReader.TryGetSize(png, out int w, out int h));
            Assert.Equal(200, w);
            Assert.Equal(100, h);

            var settings = new TimeTagTool.Core.Project.AppSettings { FontSizePx = 80 };
            var emoji = new[]
            {
                new TimeTagTool.Core.Model.EmojiEntry
                {
                    ReplaceChar = "（幅広）",
                    ImageBefore = png,
